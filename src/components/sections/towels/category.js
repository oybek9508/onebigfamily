...
const generateProductUrl = (title) => {
	...
	return `/towels/${slug}/#${slug}`;
};

const TowelCategory = () => {
	const memoizedTowelsData = useCallback(terryTowelData, []);
	console.log("memoizedTowelsData", memoizedTowelsData);
	return (
		<Box sx={{ py: { xs: "40px", md: "80px" }, px: { xs: "40px", xl: 0 } }}>
			...
			<Grid container mb={-0.5} spacing={3}>
				{memoizedTowelsData?.map((item, ind) => {
